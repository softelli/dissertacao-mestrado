...
class circum:
	...
	def getVelocities(self, numRobot , myVelocities, targetCoord, robotCoord, alienCoord, now):
		...
		if(self.hasTarget and self.hasRobot): #Escorting
			...
			#self.circularCoefToTarget = self.getCircularCoefToTarget()
			...
			#self.linearCoefTargetToRobot = self.getLinearCoefTargetToRobot(targetCoord, robotCoord)
			#-comentado anteriormente-#self.linearVelocity = sp.linear_velocity + sp.linear_velocity * self.linearCoefTargetToRobot * self.proximityCoefToRobot
			...
			#self.angularVelocity  = self.linearVelocity / sp.desired_radius + self.circularCoefToTarget
			#---fim do bloco comentado ---
		elif(self.hasTarget and not self.hasRobot): #Circulating
			self.status = 2
			circRadiusCoef, appRadiusCoef = self.getTransitionCoefs()
			kpA = self.getPropAngularCoefToTarget()
			...
			hard_lim_ang_vel = self.getHardLimited(sp.min_angular_velocity, ang_vel, sp.max_angular_velocity) + kpA * sp.delta_angular
			self.angularVelocity = self.getHardLimited(sp.min_angular_velocity, hard_lim_ang_vel,sp.max_angular_velocity)
			log.info(str(now))
			#print("kpA: %.3f" % kpA)
			#print("error_radius: %.3f" % error_radius)
			#print("appRadius * appRadiusCoef = %.3f * %.3f = %.3f" % (appRadius, appRadiusCoef, appRadius * appRadiusCoef))
			#print("circRadius * circRadiusCoef = %.3f * %.3f = %.3f" % (circRadius,circRadiusCoef,circRadius * circRadiusCoef))
			#print("radius %.3f, ang_vel: %.3f, hard_lim_ang_vel: %.3f, angularVelocity: %.3f" % (radius, ang_vel, hard_lim_ang_vel, self.angularVelocity))

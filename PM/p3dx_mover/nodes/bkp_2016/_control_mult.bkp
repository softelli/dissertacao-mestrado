...
def getLaser(data,p):
	#print "getLaser for p=", p
	global robots, beacons, aliens
	a = 0
	b = 0
	r = 0
	tmp = 0
	...
	for obj in lista:
		...
		if(obj.isRobot()):
			...
			#senao, jah existe robot detectado
				...
				if(obj.center.radius < robots[p].center.radius):
					#esse eh o atual
					robots[b] = obj
					r += 1
					#print "+1 obj robot"
			...
			if(obj.center.radius <= aliens[p].center.radius):
				aliens[b] = obj
			#	print "+1 obj alien"
			#	alien.prt()
			#	a += 1
...
def getRealTwist(odom,p):
	global realTwists
	
	realTwists[p] = odom.twist.twist
	#print "twist for p = ", p, realTwists[p]  

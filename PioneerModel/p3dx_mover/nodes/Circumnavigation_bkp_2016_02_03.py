...
class circum:
	...
	def getApproachRadius(self):
		radius = 0.0
		if self.beacon.angular <= 0:
		    #raio deve se manter negativo para a conversao
		    if self.beacon.x <= sp.desired_radius and self.beacon.y <= sp.desired_radius:
			#retorna o raio minimo em sentido de giro negativo
			radius = self.linearVelocity / sp.min_angular_velocity
		    else:
			#radius = -0.001
			#--- OK esta mantendo para esse conjunto --- #
		...
		elif 0 < self.beacon.angular and self.beacon.angular < 90.0:
		    if self.beacon.y < sp.desired_radius:
			#radius = -2.0
			#--- OK esta mantendo para esse conjunto ---#
			if self.beacon.x < sp.desired_radius:
			    radius = -(self.beacon.x**2 + self.beacon.y**2 - sp.desired_radius**2) / (2 * (sp.desired_radius - self.beacon.y))
			else:
			    radius = (self.beacon.x**2 + self.beacon.y**2 - sp.desired_radius**2) / (2 * (sp.desired_radius + self.beacon.y))
		    elif self.beacon.y == sp.desired_radius:
		        #--- OK valor fixo de 0.0 pois eh uma reta --- #
			radius = 0.0
		    else:
			#radius = 0.1
			radius = -(self.beacon.x**2 + self.beacon.y**2 - sp.desired_radius**2) / (2 * (sp.desired_radius - self.beacon.y))
		    
		elif self.beacon.angular == 90:
		    if self.beacon.y < sp.desired_radius:
			#deve retornar radius > rd
			#radius = 1.2
			#--- Ok esta gerando um raio maior, mas nao eh tangente ---
			#radius = ((self.beacon.x + sp.desired_radius)**2 + self.beacon.y**2)/(2 * self.beacon.y)
			radius = -(self.beacon.x**2 + self.beacon.y**2 - sp.desired_radius**2) / (2 * (sp.desired_radius - self.beacon.y))
		    ...
		    #deve retornar um raio positivo
		    #radius = 0.11
		    #radius = -(self.beacon.x**2 + self.beacon.y**2 - sp.desired_radius**2) / (2 * (sp.desired_radius - self.beacon.y))
		    #radius = (self.beacon.x**2 + self.beacon.y**2 - sp.desired_radius**2) / (2 * (sp.desired_radius + self.beacon.y))
		    if(self.beacon.y == sp.desired_radius):
			#retorna o raio minimo negativo
			radius = self.linearVelocity/sp.min_angular_velocity
		    else:
			radius = -(self.beacon.x**2 + self.beacon.y**2 - sp.desired_radius**2) / (2 * (sp.desired_radius - (self.beacon.y**2)**0.5))

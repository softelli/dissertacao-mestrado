...
class LocalObject:
	...
	def identify(self, beacon_radius, robot_radius, radius_tol):
		#Entradas
		...
		self.typ = self.object_types[0] #unknown, por default
		
		if(self.radius > max_robot_radius or self.center.radius <= self.p2.radius):			
			#o raio eh maior do que o esperado ou menor do que o p2 (sugere arco convexo)
			self.typ = self.object_types[3] #provavelmente um alien
		else:
			#o arco eh concavo e o raio esta dentro do limit para ser um robot ou beacon
			
			if(self.radius >= min_robot_radius):
				#se for maior que o raio minimo do robo				
				self.typ = self.object_types[2] #robot
			else:
				#eh menor que o minimo do robo				
				if(self.radius < max_beacon_radius and self.radius >= min_beacon_radius):
					
					#raio esta dentro dos limits para ser um beacon
					self.typ = self.object_types[1]

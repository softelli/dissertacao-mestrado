...
def getLight(data,nRob):
	global light_factors
	light_factors[nRob] = data.illuminance
	print("---------------------    light_factor for p%2d = %.5f" % (nRob,light_factors[nRob]))
...
laser_subscribers = []
light_subscribers = []
light_factors = [] #valores do light_sensor
...
def main(argv):
	...
	for numRobot in range(0, quantRobots):
		...
		big_led_publishers.append(rospy.Publisher(strNumRobot + "/big_led_color", Float32, queue_size = 1))
		### necessario? twist_subscribers.append(rospy.Subscriber(strNumRobot + "/cmd_vel", Twist, getTwist))
		...
		#11dez2018
		light_subscribers.append(rospy.Subscriber(strNumRobot + "/light_sensor/illuminance", Illuminance, getLight, callback_args=numRobot))
		light_factors.append(0.0)
	...
	while not rospy.is_shutdown():
		...
		for numRobot in range(0, quantRobots):
			...
			if targets[numRobot].isTarget():
				...
				#bea.prn()
				color_msg = white_color
				
			else:
				targets_coords[numRobot].clear()
				debug_msgs[numRobot] = "|        |"
				color_msg = black_color
				
		        big_led_publishers[numRobot].publish(color_msg)
			...
			ctrlTwists[numRobot].linear.x, ctrlTwists[numRobot].angular.z = ctrlCircum[numRobot].getVelocities(numRobot, velocities[numRobot], targets_coords[numRobot], robots_coords[numRobot], aliens_coords[numRobot], light_factors[numRobot], now).getVelocities()
			#twist.linear.x = 0.25			
